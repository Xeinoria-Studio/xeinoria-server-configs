...
*.yml, eula.txt, config/paper-world-defaults.yml, **server-icon.png**).
...
## Workflow
This repo is **versioning only** — there is **no auto-deploy to the live servers**, because Paper config changes typically require a server restart. The webhook does sync `server-icon.png` and the safe config files into each live server folder when this repo is pushed (icons take effect after the next server restart).

To capture the current live state into the repo: run [build.sh](./build.sh).
To apply a config change from the repo to a server: copy the file by hand and restart the server. To change a server icon: replace `<server>/server-icon.png` (64×64 PNG), push — the webhook syncs it; restart the server to apply.
